  ...
  function spawnAgent(
    ...
    return new Promise<void>((resolve) => {
      ...
      proc.on("close", (code) => {
        ...
        ctx.ui.notify(
          `Subagent #${state.id} ${state.status} in ${Math.round(
            state.elapsed / 1000,
          )}s`,
        ...
        pi.sendMessage(
          {
            ...
            content: `Subagent #${state.id}${
              state.turnCount > 1 ? ` (Turn ${state.turnCount})` : ""
            } finished "${prompt}" in ${Math.round(
              state.elapsed / 1000,
            )}s.\n\nResult:\n${result.slice(0, 8000)}${
              result.length > 8000 ? "\n\n... [truncated]" : ""
            }`,
  ...
  pi.registerTool({
    ...
    execute: async () => {
      ...
      const list = Array.from(agents.values())
        .map(
          (s) =>
            `#${s.id} [${s.status.toUpperCase()}] (Turn ${s.turnCount}) - ${
              s.task
            }`,
        )
        .join("\n");
    ...
    handler: async (_args, ctx) => {
      ...
      const msg =
        total === 0
          ...
          : `Cleared ${total} subagent${total !== 1 ? "s" : ""}${
