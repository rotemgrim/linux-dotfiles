...
import { applyExtensionDefaults } from "../themes/themeMap.ts";
    ...
    ctx.ui.setFooter((tui, theme, footerData) => {
      ...
      return {
        ...
        render(width: number): string[] {
          ...
          // Helper to calculate cost from tokens if not provided
          const calculateCost = (m: AssistantMessage): number => {
            // If cost is already calculated and non-zero, use it
            if (m.usage.cost?.total && m.usage.cost.total > 0) {
              return m.usage.cost.total;
            }

            // Otherwise, calculate from tokens and model pricing
            // Get the model's cost configuration
            // const model = ctx.models.find((mdl) => mdl.id === m.model);
            if (!model?.cost) return 0;

            const { input, output, cacheRead, cacheWrite } = model.cost;
            const inputCost = ((m.usage.input || 0) * input) / 1_000_000;
            const outputCost = ((m.usage.output || 0) * output) / 1_000_000;
            const cacheReadCost =
              ((m.usage.cacheRead || 0) * cacheRead) / 1_000_000;
            const cacheWriteCost =
              ((m.usage.cacheWrite || 0) * cacheWrite) / 1_000_000;
          ...
          for (const entry of ctx.sessionManager.getBranch()) {
            if (
              ...
              tokCacheRead += m.usage.cacheRead || 0;
              tokCacheWrite += m.usage.cacheWrite || 0;
